...
pub struct Avc420Region {
    ...
    pub top: u16,
    /// Right edge of the region (exclusive, one pixel past the region)
    pub right: u16,
    /// Bottom edge of the region (exclusive, one pixel past the region)
    pub bottom: u16,
...
impl Avc420Region {
    ...
    /// Convert to the historical `InclusiveRectangle` storage type for PDU
    /// encoding. MS-RDPEGFX defines the stored right/bottom values as exclusive;
    /// the type name predates that protocol correction.
    #[must_use]
