...
mod capability_negotiation_tests {
    ...
    #[test]
    fn avc_region_and_destination_keep_exclusive_full_frame_bounds() {
        let region = Avc420Region::full_frame(1920, 1080, 22);
        let wire_region = region.to_rectangle();
        assert_eq!((wire_region.left, wire_region.top), (0, 0));
        assert_eq!((wire_region.right, wire_region.bottom), (1920, 1080));

        let destination = GraphicsPipelineServer::compute_dest_rect(&[region], 1920, 1080);
        assert_eq!((destination.left, destination.top), (0, 0));
        assert_eq!((destination.right, destination.bottom), (1920, 1080));
    }
...
impl GraphicsPipelineServer {
    ...
    ///
    /// `Avc420Region` follows the wire format's exclusive right/bottom bounds,
    /// so its bounding box can be used directly as the destination rectangle.
